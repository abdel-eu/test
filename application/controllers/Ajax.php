	...
	public function addToCart()
	{
		...
		if(count($info) > 0)
		{
			...
			if(is_null(get_cookie("cart"))){
				...
				{
					$op = json_encode($_POST['option'], JSON_UNESCAPED_UNICODE);
				}else {
					$op = json_encode(array());
				}

				$arr[$id] = array("name" => $title, "id" => $id, "price" => $price, "img" => $img, "q" => $q, "op" => $op);
				...
				$_POST['option'] = $_POST['option'] ?? array();

				if(array_key_exists($id, $arr) && $arr[$id]["op"] == json_encode($_POST['option'], JSON_UNESCAPED_UNICODE))
				{
					$q = $q + $arr[$id]["q"];

					$arr[$id] = array("name" => $title, "id" => $id, "price" => $price, "img" => $img, "q" => $q, "op" => json_encode($_POST['option'], JSON_UNESCAPED_UNICODE));
				}else {
					$arr[$id] = array("name" => $title, "id" => $id, "price" => $price, "img" => $img, "q" => $q, "op" => json_encode($_POST['option'], JSON_UNESCAPED_UNICODE));
				}
				$ret['info'] = array("name" => $title, "id" => $id, "price" => $price, "img" => $img);
